...
def handle_income_songs(songs_q):
    while True:
        data, ip = songs_q.get()
        print("in handle songs ", ip, data)
        op_code, song_name = protocol_client.unpack_msg(data)
        if op_code == "04":
            server_song_comm.send_song(song_name[0], ip)
...
if __name__ == '__main__':
    ...
    server_song_comm = serverComm.ServerComm(2000, songs_q)
    ...
    # USER-DATA:
    client_comm.send(protocol_client.pack_signUp("reut", "123456", get_macAddress()))
    client_comm.send(protocol_client.pack_login("reut", "123456", get_macAddress()))
